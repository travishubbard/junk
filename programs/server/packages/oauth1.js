...
OAuth._requestHandlers['1'] = function (service, query, res) {                                         // 4
  ...
  if (query.requestTokenAndRedirect) {                                                                 // 15
    ...
    var callbackUrl = OAuth._redirectUri(service.serviceName, config, {state: query.state});           // 17
                                                                                                       // 18
    // Get a request token to start auth process                                                       // 19
    oauthBinding.prepareRequestToken(callbackUrl);                                                     // 20
                                                                                                       // 21
    // Keep track of request token so we can verify it on the next step                                // 22
    OAuth._storeRequestToken(                                                                          // 23
      OAuth._credentialTokenFromQuery(query),                                                          // 24
      oauthBinding.requestToken,                                                                       // 25
      oauthBinding.requestTokenSecret);                                                                // 26
                                                                                                       // 27
    // support for scope/name parameters                                                               // 28
    var redirectUrl = undefined;                                                                       // 29
    if(typeof urls.authenticate === "function") {                                                      // 30
      redirectUrl = urls.authenticate(oauthBinding);                                                   // 31
    } else {                                                                                           // 32
      // Parse the URL to support additional query parameters in urls.authenticate                     // 33
      var redirectUrlObj = url.parse(urls.authenticate, true);                                         // 34
      redirectUrlObj.query = redirectUrlObj.query || {};                                               // 35
      redirectUrlObj.query.oauth_token = oauthBinding.requestToken;                                    // 36
      redirectUrlObj.search = '';                                                                      // 37
      // Reconstruct the URL back with provided query parameters merged with oauth_token               // 38
      redirectUrl = url.format(redirectUrlObj);                                                        // 39
    }                                                                                                  // 40
                                                                                                       // 41
    // redirect to provider login, which will redirect back to "step 2" below                          // 42
    res.writeHead(302, {'Location': redirectUrl});                                                     // 43
    res.end();                                                                                         // 44
  } else {                                                                                             // 45
    // step 2, redirected from provider login - store the result                                       // 46
    // and close the window to allow the login handler to proceed                                      // 47
                                                                                                       // 48
    // Get the user's request token so we can verify it and clear it                                   // 49
    var requestTokenInfo = OAuth._retrieveRequestToken(                                                // 50
      OAuth._credentialTokenFromQuery(query));                                                         // 51
                                                                                                       // 52
    if (! requestTokenInfo) {                                                                          // 53
      throw new Error("Unable to retrieve request token");                                             // 54
    }                                                                                                  // 55
                                                                                                       // 56
    // Verify user authorized access and the oauth_token matches                                       // 57
    // the requestToken from previous step                                                             // 58
    if (query.oauth_token && query.oauth_token === requestTokenInfo.requestToken) {                    // 59
                                                                                                       // 60
      // Prepare the login results before returning.  This way the                                     // 61
      // subsequent call to the `login` method will be immediate.                                      // 62
                                                                                                       // 63
      // Get the access token for signing requests                                                     // 64
      oauthBinding.prepareAccessToken(query, requestTokenInfo.requestTokenSecret);                     // 65
                                                                                                       // 66
      // Run service-specific handler.                                                                 // 67
      var oauthResult = service.handleOauthRequest(oauthBinding);                                      // 68
                                                                                                       // 69
      var credentialToken = OAuth._credentialTokenFromQuery(query);                                    // 70
      credentialSecret = Random.secret();                                                              // 71
                                                                                                       // 72
      // Store the login result so it can be retrieved in another                                      // 73
      // browser tab by the result handler                                                             // 74
      OAuth._storePendingCredential(credentialToken, {                                                 // 75
        serviceName: service.serviceName,                                                              // 76
        serviceData: oauthResult.serviceData,                                                          // 77
        options: oauthResult.options                                                                   // 78
      }, credentialSecret);                                                                            // 79
    }                                                                                                  // 80
                                                                                                       // 81
    // Either close the window, redirect, or render nothing                                            // 82
    // if all else fails                                                                               // 83
    OAuth._renderOauthResults(res, query, credentialSecret);                                           // 84
  }                                                                                                    // 85
};                                                                                                     // 86
                                                                                                       // 87
/////////////////////////////////////////////////////////////////////////////////////////////////////////
